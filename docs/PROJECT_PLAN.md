# 📅 Project Plan: Chronos (High-Impact Scheduling)
...
- **Goal**: Clean Architecture ကို အသုံးပြုပြီး ပိုမိုကောင်းမွန်တဲ့ UI/UX နဲ့ စနစ်တကျ ရေးသားထားတဲ့ **Chronos** app တစ်ခု ဖန်တီးရန်။
...
- **Persistence**: Firebase Firestore (Current) - Real-time sync, user-based data isolation.
...
- [x] **Global Branding**: Dedicated **Chronos** Logo, Favicons (SVG), and Title implemented.
- [x] **Modern Navbar**: Sticky glassmorphism navigation with mobile support and profile slots.
- [x] **Navigation Workspaces**: Fully functional `Dashboard`, `Analytics`, and `History` screens with real browser navigation.
- [x] **Task Lifecycle History**: Added completed/archive flows so history is backed by real data instead of placeholder content.

## 🚀 Phase 4: Advanced Features (Backend Integration)

- [x] **Firebase Firestore**: LocalStorage ကနေ Firestore သို့ migrate လုပ်ခြင်း။
- [x] **Anonymous Authentication**: Firebase Anonymous Auth သုံးပြီး user isolation လုပ်ခြင်း။
- [x] **User-based Data Isolation**: Each user's data is completely isolated using `userId` field.
- [ ] **Email/Google Authentication**: Upgrade from Anonymous Auth to proper Sign In/Sign Up (Optional).
- [ ] **Reminders/Notifications**: Task စခါနီးတွင် browser notifications ပေးခြင်း။
- [x] **Search & Filter**: Global search now works across active tasks, analytics, and history records.
- [x] **Data Syncing**: Firestore ကြောင့် device အစုံတွင် data များ အလိုအလျောက် sync ဖြစ်သည်။
...
- **Deployment**: Vercel တွင် GitHub main branch မှ auto-deploy လုပ်မည်။ Repo root directory = project root (flat structure, no monorepo workspace).
- **Vercel Config**: `vercel.json` (root) — `buildCommand: npm run build`, `outputDirectory: dist`, `framework: vite`.
...
1. [x] **UI/UX Polish**: Refine the user interface before production release. (Floating Stats Animations implemented)
2. [x] **Loading States**: Add loading indicators during Firebase operations. (Premium Glassmorphism Skeletons implemented)
3. **Error Handling**: Add user-friendly error messages for Firebase errors.
4. **Email/Google Auth**: Upgrade from Anonymous Auth to proper Sign In/Sign Up (Optional).
5. **Notifications**: Build reminder logic on top of the new task lifecycle system.
6. **Drag & Drop**: Implement task reordering via drag-and-drop.
